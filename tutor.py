...
1) Tu ne réponds QU'aux questions liées, même partiellement, aux matières suivantes :
   — Informatique et génie logiciel : SQL, Python, Java, JavaScript, UML, analyse et conception UML,
     ...
     laboratoire Internet, systèmes distribués, base de données avancées, introduction à l'analyse des données (datamining),
     conception et implémentation d'applications objets, analyse des besoins et spécifications logiciels,
     contrôle qualité et métrique du logiciel, génie logiciel orienté objet, développement web avancé (web services),
     sécurité des réseaux informatiques, sécurité avancée des réseaux, administration avancée réseaux, cloud computing,
     cryptographie et sécurité des systèmes d'information, modélisation et conception de bases de données objets,
     atelier de génie logiciel, théorie des graphes, analyse numérique, programmation linéaire, électronique,
     électronique numérique, systèmes d'information, exploitation de bases de données relationnelles,
     programmation orientée objet en Java, programmation modulaire en C, atelier de programmation ;
   — Mathématiques : analyse, probabilité et statistique, algèbre, systèmes de numération, analyse numérique, théorie des graphes, programmation linéaire ;
   — Physique et électronique : mécanique générale, électricité générale, technologie des composants électroniques, optique géométrique ;
   — Langues et gestion : anglais, arabe, gestion d'entreprise, entrepreneuriat, compétences entrepreneuriales, comptabilité, économie, organisation et gestion des entreprises, droit de travail, gestion de projet, technique d'expression.
...
def _is_gemini_quota_error(exc: genai_errors.APIError) -> bool:
    """True si l'erreur ressemble à un dépassement de quota / rate limit côté Google."""
    code = getattr(exc, "code", None)
    if code == 429:
        return True
    blob = (getattr(exc, "message", None) or str(exc)).lower()
    return "quota" in blob or "rate limit" in blob or "resource exhausted" in blob
...
def _gemini_error_message(exc: genai_errors.APIError) -> str:
    ...
    if code == 429 or _is_gemini_quota_error(exc):
        return (
            "Quota ou limite Google Gemini atteinte (HTTP 429). "
            "Sur le palier gratuit, les quotas sont par modèle et par minute : attendez un peu, "
            "ou changez GEMINI_MODEL dans .env (ex. gemini-2.0-flash-lite ou gemini-2.5-flash). "
            "Suivi d’usage : https://ai.dev/rate-limit — détails : https://ai.google.dev/gemini-api/docs/rate-limits "
            f"— Message API : {msg}"
        )
    ...
    for model_id in models:
        try:
            ...
            if _is_gemini_quota_error(exc) and model_id != models[-1]:
                logger.warning(
                    "Gemini quota / 429 sur le modèle %s, essai du modèle suivant…",
                    model_id,
                )
                continue
